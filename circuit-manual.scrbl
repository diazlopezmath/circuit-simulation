...
@nb{@nbr[F] represents false}.
@nb{@nbr[T] represents true.}
A gate output is indeterminate if one or more of the inputs are indeterminate
and various determinate outputs can be expected when replacing the indeterminate inputs
by various determinate ones.
For example, an @nbr[And] gate gives indeterminate output if one input is @nbr[T]
and the other one is indeterminate, for in this case the output depends on which determinate signal
will be given for the indeterminate input.
...
A gate output can be connected via a @seclink["wires"]{wire} to the inputs of one or more other
gates or even to an input of the same gate itself.
...
A gate output cannot be an external input wire.
...
@nb{A wire} preserves its signal as long as not mutated.
...
The @italic{@tt{description}} can be written by straightforwardly listing the elements of
a diagram of the circuit.}

@item{
@tt{(@nbr[define]@(hspace 2)input-wire (@nbr[wire-make] @italic{name} [@italic{signal}])) ...}
and@(lb)
...
The constructor returns @(Void), but it does make a circuit behind the screen
...
id est switching the signals on one or more wires ad infinitum, thus causing an infinite loop.
Parameter @nbr[agenda-time-limit] can protect against such infinite loops:@(lb)
...
Simply typing the identifier of a wire will do too. A wire is written, printed or displayed as
@tt{#<wire:@italic{name=signal}>}.}

@item{Stop here or repeat from step 4.}]
...
Once the signals on wires @tt{state} and @tt{state-inverse} are inverses of each other,
they always end up as inverses of each other.
...
The initial signal on the four wires is @nbr[F].
...
on the @tt{clock} from time 10 to time 20.
No simulation yet.
For times @nbrl[natural?]{natural numbers} are used without specifying a unit.
This unit always is implied and always the same.

@Interaction*[
(code:comment "At power up time the clock has signal F.")
(code:line (agenda-schedule! clock-wire T 10) (code:comment "raise signal to T at time 10"))
(code:line (agenda-schedule! clock-wire F 20) (code:comment " drop signal to F at time 20"))]
...
(code:comment "Apply a T pulse on the clock-wire for 5 units of time.")
...
@Interaction*[
(define (test-and-tabulate flip-flop-constr tabulate?)
 (code:comment " ")
...
(code:comment " ")
(test-and-tabulate D-flip-flop-constr #t)]

The two @nbr[Nand] gates at the right of the @elemref["D-flip-flop-diagram"]{diagram}
form a kind of SR-latch,@(lb)
but compared to a regular SR-latch it interprets its S and R input as their inverses.@(lb)
...
@deftogether[
(@defthing[#:kind "false/off/low, constant" F trit?]
 @defthing[#:kind "true/on/high, constant" T trit?]
...
They are trits too.@(lb)
@nbr[?] is a trit, but not a binary digit.
...
When an event is scheduled for the same wire and time as already scheduled,
@nb{the previous} one is canceled.
...
the gate does not schedule an event for its output.
...
Procedure @nbr[agenda-schedule!] cannot ignore events that in future will not change the signal on
@nb{a wire}, because without actually continuing the simulation,
in general it cannot know which signal the wire will have when the time of the event will come.
...
@defproc[(agenda-schedule!
          (wire wire?)
          (signal trit?)
          (delay natural? F)) void?]{

Schedules in the @nbr[current-agenda] a @nbr[signal] change at time @nbr[(+ delay (agenda-time))]
for the @nbr[wire].
...
A wire is hidden if its name ends with a dot ‘•’ (@tt{#\u2022})
...
Does not affect the identity of hidden wires. Distinct wires can have the same name.}
...
@defform-remove-empty-lines[@defform[(define-wires def ...)
#:grammar
((def id (id) (id signal) (id name signal) (id restricted-name)))
#:contracts ((name symbol?) (signal trit?) (restricted-name (and/c symbol? (not/c trit?))))]{
                                             
Same as:

@inset{@nbr[(define-values (id ...) (values (wire-make name signal) ...))]}

In the @nbr[def]s the default @nbr[name] is @tt{'}@nbr[id] and the default signal is
...
(define-wires
 ...
 (wire-c 'c)
 (d T)
 (wire-e (car '(e)) (Not-function T)))
(wire-println a b wire-c d wire-e)]}]
...
• No duplicates are allowed in any of the following parts of the form:@(lb)
@(hspace 4)@nbr[(input-wire ...)]@(lb)
@(hspace 4)@nbr[(output-wire ...)]@(lb)
@(hspace 4)@nbr[(sub-output-wire ... ...)]@(lb)
...
can have one output only, for which a hidden internal wire is incorporated.
...
unless reset by means of procedure @nbr[reset-hidden-cntr!].@(lb)
...
@defproc[#:kind "gate constructor" (If (test wire?) (then wire?) (else wire?) (out wire?))
          void?]{
An @nbr[If] gate puts the following signal on wire @nbr[out]:@(lb)
@(hspace 3)the signal of wire @nbr[then] if that of wire @nbr[test] is @nbr[T],@(lb)
@(hspace 3)the signal of wire @nbr[else] if that of wire @nbr[test] is @nbr[F],@(lb)
@(hspace 3)the signal of wire @nbr[then] if wires @nbr[then] and @nbr[else] have the same signal,@(lb)
@(hspace 3)@nbr[?] in all other cases.
...
Lines with determinate inputs only precede all lines @nb{with an} indeterminate input.

@(make-truth-tables (a) (Not Not-function))
...
A @nbr[Delay] gates can be used for synchronization.
...
This seems to work,
but see what happens after dropping the @tt{clock} to @nbr[F]:
...
(for*/and ((n (in-range -32 32)) (m (in-range -32 32)))
 (agenda-reset!)
 (define a (number->bit-list n))
 (define b (number->bit-list m))
 (for ((a (in-list (list a5 a4 a3 a2 a1 a0)))
       (bit (in-list (number->bit-list n))))
...
one with @tt{clock=1} to set or reset or flip the first SR-flip-flop or to leave it as it is and
@nb{one call} with @tt{clock=0} in order to copy the state of the first SR-flip-flop
into the second one.
@tt{clock=1} may change the first SR-flip-flop but leaves the second one unaffected.
@tt{clock=0} leaves the first SR-flip-flop unaffected and
